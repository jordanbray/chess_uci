...
where
    ...
    fn new_mate(ply: i16, color: Color) -> Self {
        match color {
            ...
            Color::Black => Self::min_value() + Self::from_ply(ply) + Self::one(),
        }
    }
    ...
    fn depth_to_mate(&self) -> Option<i64> {
        if *self > Self::max_value() - Self::max_supported_mates() {
            ...
            Some((Self::min_value() - *self + Self::one()).to_i64().expect("Failed to convert mate depth to i64"))
        } else {
            None
        }
    }
...
fn test_mates<T: Eval>() {
    assert_eq!(T::min_value() + T::one(), T::new_mate(0, Color::Black));
